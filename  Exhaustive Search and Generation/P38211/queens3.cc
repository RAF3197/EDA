...
void reines(int fila,int n,vector<int>& solucio,vector<bool>& col,vector<bool>& diagonal1,vector<bool>& diagonal2,bool& escrit,bool& trobat){
  if (fila==n){escriu(solucio);trobat=true;}
  else{
    for (int columna=0;columna<n;++columna){
      int diag1=n-columna-1+fila;
      int diag2=columna+fila;
      if (not col[columna] and not diagonal1[diag1] and not diagonal2[diag2] and solucio[fila]==-1 and not trobat){
        solucio[fila]=columna;
        col[columna]=true;
        diagonal1[diag1]=true;
        diagonal2[diag2]=true;
        reines(fila+1,n,solucio,col,diagonal1,diagonal2,escrit,trobat);
        col[columna]=false;
...
int main(){
  ...
  bool escrit=false;
  bool trobat=false;
  reines(0,n,solucio,columna,diagonal1,diagonal2,escrit,trobat);
